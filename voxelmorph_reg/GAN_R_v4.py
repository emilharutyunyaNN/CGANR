"""
Possible Problems: 
    - Affine not correctly integrated, although version 3 works
    - G and D models nested and too big, so training with the 
    iteration counts as in AutopsyVirtual lead to GPU getting filled up.
        *** Distributed training with Lightning torch
        *** changes unet_att_2d and discriminator_2d implementations: models initialized during forward passes
"""
...
### TODO: 1. memory fraction monitoring as a possible solution
memory_fraction = 0.5
...
class PatchDataset(Dataset):
    def __init__(self, dir1, dir2, indices = None, patch_size=256, stride=256, transform=None, num_workers = 4):
        self.dir1 = dir1
        self.dir2 = dir2
...
model_G = att_unet_2d(input_size=(3, 256, 256), filter_num=filters, n_labels=3, rank=1)
#model_G = DDP(model_G, device_ids=[rank])
model_G.to(device)

model_D = discriminator_2d(input_size=(3, 256, 256), filter_num=filters, rank = 1, stack_num_down=2, activation='ReLU', batch_norm=False, pool=False, backbone=None)
...
def R_affine_train(input_image, target):
    ...
    with autocast():
        ...
        with torch.no_grad(): R_outputs, _ = model_R(R_outputs, G_outputs)
