...
import sys
"""
works with batch_utils_v10 which loads from precreated hdf5 file dataset, is fast also uses DIST loss instead of huber for the first epoch at least, R still uses NCC.
"""
...
def init_parameters():
    ...
    tc.q_limit, vc.q_limit = 100, 10 # 300->10
...
# Create training and validation dataset instances
   ...
   # infinite_train_dataloader = InfiniteDataLoader(train_dataloader)
    ...
    def G_train_step(input_image, target, epoch):
        ...
        G_total_loss, G_dis_loss, G_l1_loss = loss_G_perceptual(D_fake_output, G_outputs, target_transformed,tc, loss_dist,epoch)
    ...
    def G_test_step(input_image, target, epoch):
        ...
        with torch.no_grad():
            D_fake_output = model_D(G_outputs)
        G_total_loss, G_dis_loss, G_l1_loss = loss_G_perceptual(D_fake_output, G_outputs, target, tc, loss_dist,epoch, train=False)
