...
model_G = att_unet_2d(input_size=(3, 256, 256), filter_num=filters, n_labels=3, rank=1).to(device)
#model_G = DDP(model_G, device_ids=[rank])

model_D = discriminator_2d(input_size=(3, 256, 256), filter_num=filters, rank = 1, stack_num_down=2, activation='ReLU', batch_norm=False, pool=False, backbone=None).to(device)
...
"""if __name__ == "__main__":
    ...
    dataset = PatchDataset(dir1, dir2, patch_size=1024, stride=1024)
    #world_size = torch.cuda.device_count()
    #mp.spawn(main, args = (world_size,dataset,), nprocs=world_size, join=True)
