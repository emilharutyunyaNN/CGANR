...
def generate_patch(file, image_size, is_mat = True, inpnorm = 'norm_by_mean_std', start = 0, end = 3, gray = False):
        s = image_size
        print(gray)
        stride = s
        ...
        if is_mat:
            ...
            try:
                ...
                #print(image.shape)
                    ...
                    print("--")
                    image = cv2.imread(file.replace("target", "input"), cv2.IMREAD_GRAYSCALE).astype(np.float32)/255.0
                    label = cv2.imread(file, cv2.IMREAD_GRAYSCALE).astype(np.float32)/255.0
            except Exception as e:
                print(f"Error loading {file}: {e}")
                return
...
def generate_patch_wrapper(path, image_size, is_mat, gray):
    print("-", gray)
    def generator():
        yield from generate_patch(file = path, image_size = image_size, is_mat = is_mat, gray = gray)
    return generator
...
class TransformImageBatchLoader:
    def __init__(self, images, config, input_channels, is_testing, n_parallel_calls, q_limit, n_epoch, gray = False,gen_batch=10):
    ...
    def dynamic_dataset_generator(self, batch_size=10, reshuffle = False):
        if not self.gray:
            ...
            features = Features({
                'input': Array3D(dtype='float32', shape=(256, 256, 1)),
                'target': Array3D(dtype='float32', shape=(256, 256, 1))
            })
        ...
        for batch_paths in file_path_batches():
            print("gg: ", self.gray)
            datasets = [Dataset.from_generator(generate_patch_wrapper(path, self.image_size, self.is_mat, self.gray), features=features, streaming=True) for path in batch_paths]
            ...
            for example in interleaved_dataset:
                ...
                if len(all_examples) == self.batch_size:
                    ...
                    all_examples = []
            if all_examples:
                remaining_dataset = Dataset.from_generator(regular_generator(all_examples), features=features, streaming=True)
                grad_list.append(remaining_dataset)
                all_examples = []
