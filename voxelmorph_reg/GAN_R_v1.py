...
def main(rank, world_size):
        ...
        model_G = att_unet_2d(input_size=(3, 256, 256), filter_num=filters, n_labels=3,rank=rank).to(torch.device(rank))
        model_G = DDP(model_G, device_ids=[rank])

        model_D = discriminator_2d(input_size=(3, 256, 256), filter_num=filters, stack_num_down=2, activation='ReLU', batch_norm=False, pool=False, backbone=None).to(torch.device(rank))
        ...
        model_R = VxmDense(
            ...
            trg_feats=3,
            unet_half_res=False
        ).to(torch.device(rank))
        model_R = DDP(model_R, device_ids= [rank])
        ...
        def G_train_step(input_image, target, epoch):
            input_image = input_image.to(torch.device(rank))
            target = target.to(torch.device(rank))
            ...
            print("Gtrain input: ",input_image.shape)
            ...
            print(f"model devices for process {rank}: ", next(model_D.parameters()).device, next(model_G.parameters()).device, next(model_R.parameters()).device)
            print(f"before loss devices for process {rank}: ", D_fake_output.device, G_outputs.device, target_transformed.device)
            ...
            if epoch > 0 or (tc.G_warmstart_checkpoint is None and tc.prev_checkpoint_path is None):        
                ...
                for i in tqdm(range(num_checkpoint_D)):
                    ...
                    for j in range(NumGen):
                        ...
                        print("batch shapes: ", targets.shape, input_images.shape)
