...
import torch
import glob
from IQA_pytorch import SSIM, DISTS, LPIPSvgg, MS_SSIM
torch.cuda.empty_cache()
import sys
"""
works with batch_utils_v10 which loads from precreated hdf5 file dataset, is fast also uses different perceptual losses instead of huber L1 for the first epoch at least (uses argeparse to parse the loss), R still uses NCC.
changing the R loss from ncc to dists for the first epoch
...
# Define device
from dataloader.batch_utils_v10 import *
...
def init_parameters():
    ...
    tc.q_limit, vc.q_limit = 100, 300 # 300->10
...
def run_validation(vc, epoch, iterator_valid_bl, D_test_step,G_test_step, R_test_step, use_tqdm = True):
    ...
    valid_G_total_loss_mean = np.mean(np.array([t.detach().cpu().numpy() for t in valid_G_total_loss_list]))
    if type(valid_G_l1_loss_list[0]) == float:
        valid_G_l1_loss_mean = np.mean(np.array([t for t in valid_G_l1_loss_list]))
    else:
        valid_G_l1_loss_mean = np.mean(np.array([t.detach().cpu().numpy() for t in valid_G_l1_loss_list]))
    ...
    valid_R_total_loss_mean = np.mean(np.array([t.detach().cpu().numpy() for t in valid_R_total_loss_list]))
...
if __name__ == "__main__":
    ...
    parser.add_argument("--loss", help = "perceptual loss for the first epoch because R is not being used or trained and the alignment is bad")

    args = parser.parse_args()

    loss = args.loss
    if loss == 'dists':
        model_L = DISTS().to(device)
    elif loss == 'ssim':
        model_L = SSIM(channels=3).to(device)
    elif loss == 'msssim':
        model_L = MS_SSIM(channels=3).to(device)
    elif loss == 'lpips':
        model_L = LPIPSvgg().to(device)
        
    else:
        raise argparse.ArgumentTypeError('invalid value! try --help')
    ...
    augmentation = Augmentation(tc.num_slices, tc.label_channels)
...
# Create training and validation dataset instances
    ...
    dataloader_train = DataLoader(dataset_train, batch_size=4, num_workers=4, pin_memory=True, worker_init_fn=worker_init_fn, persistent_workers=True)
    dataloader_val = DataLoader(dataset_val, batch_size=4, num_workers=4, pin_memory=True, worker_init_fn=worker_init_fn, persistent_workers=True)
   ...
   # infinite_train_dataloader = InfiniteDataLoader(train_dataloader)
    ...
    def G_train_step(input_image, target, epoch):
        ...
        G_total_loss, G_dis_loss, G_l1_loss = loss_G_perceptual_all(D_fake_output, G_outputs, target_transformed,tc, model_L,epoch)
    ...
    def G_test_step(input_image, target, epoch):
        ...
        with torch.no_grad():
            D_fake_output = model_D(G_outputs)
        G_total_loss, G_dis_loss, G_l1_loss = loss_G_perceptual_all(D_fake_output, G_outputs, target, tc, model_L,epoch, train=False)
    ...
    # print("+++", target_clipped_splitted.shape, G_output_clipped_splitted.shape)
        G_ncc = torch.mean(NCC(eps = 1e-3).loss(target_clipped_splitted, G_output_clipped_splitted))# TODO: check if correct
    ...
    def R_train_step(input_image, target, epoch):
    # train_sampler.set_epoch(epoch)
        input_image, target = input_image.to(device), target.to(device)
        model_R.train()
        model_G.train()
        ...
        R_total_loss, R_berhu_loss = loss_R_no_gt_perceptual(R_outputs, G_outputs, tc, epoch, model_L)
    ...
    def R_test_step(input_image, target, epoch):
        model_G.eval()
        model_R.eval()
        input_image, target = input_image.to(device), target.to(device)
        with torch.no_grad():
            G_outputs = model_G(input_image)
            R_outputs = model_R(target, G_outputs)
        R_total_loss, R_berhu_loss = loss_R_no_gt_perceptual(R_outputs, G_outputs, tc, epoch, model_L, False)
    ...
    for epoch in range(epoch_begin, tc.N_epoch):
        ...
        train_loader = iter(dataloader_train)
        valid_loader = iter(dataloader_val)
        ...
        if epoch > 0 or (tc.G_warmstart_checkpoint is None and tc.prev_checkpoint_path is None):        
            ...
            for i in tqdm(range(num_checkpoint_D)):
                ...
                if i%50 ==0:
                    os.makedirs(f"./voxelmorph_reg/output_image_v13/iter_{i}", exist_ok = True)
                    #print("**: ", targets[0,:,:,:].shape)
                    #print(train_G_output[0,:,:,:].shape)
                    #print("----", (targets[0,:,:,:].permute(1,2,0).detach().cpu().numpy()<0).any())
                    plt.imsave(f"./voxelmorph_reg/output_image_v13/iter_{i}/train_epoch={epoch}_sample_output_G.jpg", np.clip(train_G_output[0,:,:,:].permute(1,2,0).detach().clone().cpu().numpy(), 0, 1))
                    plt.imsave(f"./voxelmorph_reg/output_image_v13/iter_{i}/train_epoch={epoch}_sample_output_R.jpg", np.clip(train_R_output[0,:,:,:].permute(1,2,0).detach().clone().cpu().numpy(), 0, 1))
                    plt.imsave(f"./voxelmorph_reg/output_image_v13/iter_{i}/train_epoch={epoch}_sample_target.jpg", targets[0,:,:,:].permute(1,2,0).detach().clone().cpu().numpy())
                    plt.imsave(f"./voxelmorph_reg/output_image_v13/iter_{i}/train_epoch={epoch}_sampleinput0.jpg", -input_images[0,0,:,:].detach().clone().cpu().numpy(), cmap = 'gray')
            ...
            #  print("D check:", num_checkpoint_D)
                ...
                if iter_D_count % tc.valid_steps == 0 and i!= num_checkpoint_D-1:
                    ...
                    if type(train_G_l1_loss_list[0]) == float:
                        train_G_l1_loss_mean = np.mean(np.array([ t for t in train_G_l1_loss_list]))
                    else:
