...
# Create training and validation dataset instances
   ...
   # infinite_train_dataloader = InfiniteDataLoader(train_dataloader)
    ...
    def G_train_step(input_image, target, epoch):
        ...
        writer.add_scalar("GtotalLoss/train", G_total_loss, epoch)
        writer.add_scalar("GdisLoss/train", G_dis_loss, epoch)
        writer.add_scalar("GL1Loss/train", G_l1_loss, epoch)
    ...
    for epoch in range(epoch_begin, tc.N_epoch):
        ...
        if epoch > 0 or (tc.G_warmstart_checkpoint is None and tc.prev_checkpoint_path is None):        
            ...
            for i in tqdm(range(num_checkpoint_D)):
                ...
                if i%200 ==0:
                    os.makedirs(f"./voxelmorph_reg/output_image_v10/iter_{i}", exist_ok = True)
                    #print("**: ", targets[0,:,:,:].shape)
                    #print(train_G_output[0,:,:,:].shape)
                    #print("----", (targets[0,:,:,:].permute(1,2,0).detach().cpu().numpy()<0).any())
                    plt.imsave(f"./voxelmorph_reg/output_image_v10/iter_{i}/train_epoch={epoch}_sample_output_G.jpg", np.clip(train_G_output[0,:,:,:].permute(1,2,0).detach().clone().cpu().numpy(), 0, 1))
                    plt.imsave(f"./voxelmorph_reg/output_image_v10/iter_{i}/train_epoch={epoch}_sample_output_R.jpg", np.clip(train_R_output[0,:,:,:].permute(1,2,0).detach().clone().cpu().numpy(), 0, 1))
                    plt.imsave(f"./voxelmorph_reg/output_image_v10/iter_{i}/train_epoch={epoch}_sample_target.jpg", targets[0,:,:,:].permute(1,2,0).detach().clone().cpu().numpy())
                    plt.imsave(f"./voxelmorph_reg/output_image_v10/iter_{i}/train_epoch={epoch}_sampleinput0.jpg", -input_images[0,0,:,:].detach().clone().cpu().numpy(), cmap = 'gray')
